...
export default function Terminal({
  ...
  const [history, setHistory] = useState<TerminalLine[]>([
    {
      text: "TERMINAL v1.1 - Awaiting commands...",
      className: "terminal-medium-green",
    },
  ]);
  ...
  const addToHistory = (
    command: string,
    response: string,
    responseClass: string = "terminal-green"
  ) => {
    setHistory((prev) => [
      ...prev,
    ...
    switch (cmd) {
      case "buy":
        if (parts.length < 2) {
          addToHistory(
            command,
            "ERROR: Specify upgrade to buy. Usage: buy [upgrade-id] [amount]",
            "text-red-400"
          );
          return;
        }
        ...
        if (!upgrade) {
          addToHistory(
            command,
            `ERROR: Unknown upgrade "${upgradeId}". Available: ${gameState.upgrades
              .filter((u) => u.unlocked)
              .map((u) => u.id)
              .join(", ")}`,
            "text-red-400"
        ...
        if (!upgrade.unlocked) {
          addToHistory(
            command,
            `ERROR: Upgrade "${upgradeId}" not unlocked yet`,
            "text-red-400"
          );
          return;
        }
        ...
        if (gameState.money < cost) {
          addToHistory(
            command,
            `ERROR: Insufficient funds. Need $${cost - gameState.money} more`,
            "text-red-400"
          );
          return;
        }
        ...
        addToHistory(
          command,
          `SUCCESS: Purchased ${amount}x ${upgrade.name} for $${cost}`
        );
        break;
      ...
      case "research":
        if (parts.length < 2) {
          addToHistory(
            command,
            "ERROR: Specify research project. Usage: research [project-id]",
            "text-red-400"
          );
          return;
        }
        ...
        if (!research) {
          addToHistory(
            command,
            `ERROR: Unknown research "${researchId}". Available: ${gameState.research
        ...
        if (!research.unlocked) {
          addToHistory(
            command,
            `ERROR: Research "${researchId}" not unlocked yet`,
            "text-red-400"
          );
          return;
        }
        ...
        if (gameState.currentResearch) {
          addToHistory(
            command,
            "ERROR: Already researching. Wait for current research to complete",
            "text-red-400"
          );
          return;
        }
        ...
        const hasAllDeps = research.dependencies.every((dep) => {
          ...
          return (
            gameState.research.find((r) => r.id === dep)?.completed || false
          );
        });
        ...
        if (!hasAllDeps) {
          addToHistory(
            command,
            `ERROR: Missing dependencies: ${research.dependencies.join(", ")}`,
            "text-red-400"
          );
          return;
        }
        ...
        addToHistory(
          command,
          `SUCCESS: Started ${research.name} research (${Math.round(
            actualDuration
          )} seconds)`
        );
        break;
      ...
      case "transition":
        if (parts[1] === "gui") {
          ...
          if (projectGUI?.completed) {
            ...
            addToHistory(
              command,
              'ERROR: Must complete "Project GUI" research first',
              "text-red-400"
            );
          }
        } else {
          addToHistory(
            command,
            "ERROR: Unknown transition. Try: transition gui",
            "text-red-400"
          );
        }
        break;
      ...
      case "confirm-transition":
        if (onTransitionToGUI) {
          ...
          addToHistory(
            command,
            "SUCCESS: Transitioning to GUI era...",
            "terminal-green"
          );
        } else {
          addToHistory(
            command,
            "ERROR: GUI transition not available",
            "text-red-400"
          );
        }
        break;
      ...
      case "reset":
        ...
        setHistory([
          {
            text: "TERMINAL v1.1 - Game reset. Awaiting commands...",
            className: "terminal-medium-green",
          },
        ]);
        return;
      ...
      case "addmoney":
        ...
        addToHistory(
          command,
          `DEBUG: Added $${debugAmount} to your money`,
          "text-yellow-400"
        );
        break;
  ...
  return (
    <div className="terminal-border p-4 min-h-full">
      <div className="flex justify-between items-center mb-3">
        ...
        <span className="text-sm terminal-medium-green">
