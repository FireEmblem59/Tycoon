...
import KonamiDebugWindow from "./ui/KonamiDebugWindow";

interface GUIInterfaceProps {
  gameState: GameState;
  onButtonPress: () => void;
  onBuyUpgrade: (upgradeId: string, amount: number) => void;
  onStartResearch: (researchId: string, instant?: boolean) => void;
...
export default function GUIInterface({
  ...
  const [instantResearchEnabled, setInstantResearchEnabled] = useState(false);
  ...
  const guiUpgrades = gameState.upgrades.filter(
    (u) => u.era === "gui" && u.unlocked
  );
  const guiResearch = gameState.research.filter(
    (r) => r.era === "gui" && r.unlocked && !r.completed
  );
    ...
    <div className="min-h-screen bg-gray-200 font-mono overflow-auto">
      ...
      <div className="bg-gray-300 border-b-2 border-gray-600 p-2 shadow-md">
        <div className="flex justify-between items-center">
          <h1 className="text-xl font-bold text-black">
            Button Tycoon - GUI Era (1985)
          </h1>
          <div className="text-lg font-bold text-black">
            Money: ${gameState.money.toFixed(2)}
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
            ...
            <div className="p-6 text-center">
              <div className="mb-4">
                ...
                <div className="text-sm text-gray-600 mb-4">
                  Button Presses: {gameState.buttonPresses} | Auto Income: $
                  {gameState.guiAutoIncome}/sec
                </div>
              </div>
              <button
              ...
              >
                ASSEMBLE (+$
                {1 +
                  (gameState.upgrades.find((u) => u.id === "mouse-upgrade")
                    ?.owned || 0) *
                    0.5 +
                  (gameState.upgrades.find((u) => u.id === "graphics-card")
                    ?.owned || 0) *
                    2}
                )
              </button>
                      ...
                      >
                        ...
                        {!canAfford && (
                          <div className="text-red-600 text-xs mt-1">
                            Need ${(cost - gameState.money).toFixed(2)} more
                          </div>
                        )}
                      </div>
                    );
              ...
              {guiResearch.length > 0 ? (
                <div className="space-y-3">
                  {guiResearch.map((research) => {
                    const internCount =
                      gameState.upgrades.find((u) => u.id === "intern")
                        ?.owned || 0;
                    const speedMultiplier = 1 + internCount * 0.5;
                    const actualDuration = Math.round(
                      research.timeRequired / speedMultiplier
                    );
                    const isResearching =
        ...
        <div className="mt-4 bg-white border-2 border-gray-600 shadow-lg">
          <KonamiDebugWindow
            onSetMoney={(amount) => onAddMoney(amount)}
            onToggleInstantResearch={(enabled) => {
              setInstantResearchEnabled(enabled);
            }}
          />
